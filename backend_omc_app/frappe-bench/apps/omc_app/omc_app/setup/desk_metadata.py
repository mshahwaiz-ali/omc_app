...
def _hide_automatic_naming_series_fields():
    for doctype in _AUTOMATIC_NAMING_SERIES_DOCTYPES:
        field = frappe.db.get_value('DocField', {'parent': doctype, 'fieldname': 'naming_series'}, ['name', 'default', 'options'], as_dict=True)
        if not field:
            continue
...
def _remove_standalone_tax_slab_links():
    links = frappe.get_all('Workspace Link', filters={'parent': 'OMC App', 'parenttype': 'Workspace', 'link_to': 'OMC Tax Slab'}, pluck='name')
