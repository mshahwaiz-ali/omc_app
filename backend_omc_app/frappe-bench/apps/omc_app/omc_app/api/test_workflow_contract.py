...
class WorkflowContractTest(unittest.TestCase):
    def test_rejected_receipt_preserves_payment_stage_and_requests_replacement(self):
        result = workflow_contract.project({"status": "Waiting for Payment", "required_documents_count": 1, "submitted_documents_count": 1, "approved_documents_count": 0, "payments_count": 1, "open_payments_count": 1, "rejected_payments_count": 1})
    ...
    def test_completion_requires_documents_payment_and_operations(self):
        result = workflow_contract.project({"status": "In Progress", "required_documents_count": 2, "submitted_documents_count": 2, "approved_documents_count": 0, "payments_count": 1, "paid_payments_count": 1, "operational_work_complete": True})
        self.assertTrue(result["completion_eligible"])
        self.assertEqual(result["completion_blockers"], [])

    def test_uploaded_documents_complete_document_stage_without_approval(self):
        result = workflow_contract.project(
            {
                ...
                "payments_count": 1,
                "open_payments_count": 1,
            }
        )

        self.assertEqual(result["current_stage"], "payment")
        self.assertIn("documents_uploaded", result["milestones"])
        self.assertNotIn("documents_approved", result["milestones"])

    def test_missing_upload_blocks_document_completion(self):
        result = workflow_contract.project(
            {
        ...
        self.assertEqual(result["current_stage"], "documents")
        self.assertIn(
