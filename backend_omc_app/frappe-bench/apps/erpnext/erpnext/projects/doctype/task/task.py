...
class Task(NestedSet):
	...
	def get_image_data(self):
		if self.custom_proof_image and not (self.custom_registration_no or self.custom_tax_year or self.custom_barcode):
			from .read_image_data_ai import _read_file_as_base64, _call_openai_for_ack_slip

			try:
				b64, mime = _read_file_as_base64(self.custom_proof_image)
				result = _call_openai_for_ack_slip(b64, mime)
				if hasattr(self, "custom_registration_no"):
					self.custom_registration_no = result.get("registration_no")
				if hasattr(self, "custom_tax_year"):
					self.custom_tax_year = int(result.get("tax_year")) if result.get("tax_year") else None
				if hasattr(self, "custom_barcode"):
					self.custom_barcode = result.get("barcode")
				if hasattr(self, "custom_period"):
					self.custom_period = result.get("period")
				if hasattr(self, "custom_document_date"):
					self.custom_document_date = result.get("document_date")
			except Exception:
				frappe.log_error(title="ACK Slip Parse Failed", message=frappe.get_traceback())

		elif not self.custom_proof_image:
			self.custom_registration_no = None
			self.custom_tax_year = None
			self.custom_barcode = None
			self.custom_period = None
			self.custom_document_date = None
				
	def validate_data(self):
		if self.workflow_state == "Completed":
			if self.task_status == "Monthly Sales Tax":
				if str(self.get("custom_period","")) != str(self.get("custom_tax_period","")):
					self.workflow_state = "Working in Progress"
					frappe.throw(_(f"Tax Period do not match"))
				if str(self.get("custom_year","")) != str(self.get("custom_tax_year","")):
					self.workflow_state = "Working in Progress"
					frappe.throw(_(f"Tax Year {self.custom_year} and {self.custom_tax_year} do not match"))

				if self.tax_id != self.custom_registration_no:
					self.workflow_state = "Working in Progress"
					frappe.throw(_("Registration No do not match"))
			else:
				if str(self.get("custom_year","")) != str(self.get("custom_tax_year","")):
					self.workflow_state = "Working in Progress"
					frappe.throw(_(f"Tax Year {self.custom_year} and {self.custom_tax_year} do not match"))
...
def bulk_generate_invoices(tasks):
    ...
    for t in tasks:
        ...
        if doc.status == "Completed":
            ...
            inv.posting_date = doc.completed_on
            inv.company = "Omc House"
            inv.customer = doc.customer
