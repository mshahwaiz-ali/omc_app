...
frappe.listview_settings['Task'] = {
    onload: function(listview) {
        listview.page.add_menu_item(__('Generate Invoice'), function() {
            ...
            let task_names = selected.map(d => d.name);

            frappe.call({
                method: "erpnext.projects.doctype.task.task.bulk_generate_invoices",
                args: {
                    tasks: task_names
                },
                freeze: true,
                freeze_message: "Generating Invoices...",
                callback: function(r) {
                    if (r.message) {
                        frappe.msgprint(__('Invoices generated: ' + r.message.join(", ")));
                        listview.refresh();
                    }
                }
            });
