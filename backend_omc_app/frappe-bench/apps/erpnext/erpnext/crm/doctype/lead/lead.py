...
import requests
import json
from frappe.utils import now_datetime, get_datetime, getdate, today
...
class Lead(SellingController, CRMNote):
	...
	def before_save(self):
		if self.source and self.sales_person:
			try:
				# Get the sales_person doc (e.g., Consultant, Employee, etc.)
				source_doc = frappe.get_doc(self.source, self.sales_person)
				
				# Get the linked user from user_link field
				user_id = getattr(source_doc, "user_link", None)
				
				if user_id:
					# Fetch commission structure from User
					commission_structure = frappe.db.get_value("User", user_id, "commission_structure")
					
					if commission_structure:
						self.commission_structure = commission_structure

			except Exception:
				frappe.log_error(frappe.get_traceback(), "Error in Lead.before_save (commission structure fetch)")
...
@frappe.whitelist()
def fetch_zong_leads():
	call_types = ["inbound", "outbound"]
	for call_type in call_types:
		fetch_io_zong_leads(call_type)
	return "ZONG Leads Fetched Successfully."


def fetch_io_zong_leads(call_type="inbound"):	
	try:
			# Fetch CRM Settings
			crm_settings = frappe.get_single("CRM Settings")
			last_sync = crm_settings.last_sync or "2025-09-01"  # Default if not set

			# Prepare API request payload
			payload = json.dumps({
				"vpbx_id": crm_settings.vpbx_id,
				"token": crm_settings.token,
				"call_type": call_type,
				"start_date": str(getdate(last_sync)),
				"end_date": str(getdate(today()))
			})
			headers = {
			...
			response = requests.post(
				"https://cap.zong.com.pk:8444/vpbx-apis/customApi/vpbx-custom-apis",
				headers=headers,
				data=payload
			)
			...
			if response.status_code == 200:
				...
				if data.get("status") and data.get("data"):
					for call in data["data"]:
						# Check if lead exists based on customer_number
						existing_lead = frappe.db.exists("Lead", {"mobile_no": call["customer_number"]})
						if not existing_lead:
							# Create new lead
							lead = frappe.get_doc({
								"doctype": "Lead",
								"company_name": "UBL",  # Set first name as UBL
								"serives_amount": 0.00,
								"task_type": "UBL Lead",
								"mobile_no": call["customer_number"],
								"custom_mater_number": call["master_number"],
								"extension": call["extension"],
								"call_type": call["call_type"],
								"call_response": call["call_response"],
								"duration": call["duration"],
								"custom_lead_source": "Call",
								"source": "Business Partner",
								"sales_person": "ubl@omchouse.com",
								"call_time": call["time"].split(" ")[0]+" "+call["time"].split(" ")[1],  # Extract time from datetime
								"recording_link": call["recording"]
							})
							lead.insert(ignore_permissions=True)
					
					# Update last sync time in CRM Settings
					if last_sync != today():
						crm_settings.last_sync_datetime = today()
						crm_settings.save()
					
					return "Leads synced successfully."
				else:
					frappe.msgprint("No data found or API error.")
			else:
				frappe.throw("Failed to fetch data from ZONG API.")
	except:
		frappe.log_error(frappe.get_traceback(), "Error in fetch_zong_leads")
