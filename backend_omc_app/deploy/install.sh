...
m="$(free -m | awk '/^Swap:/{print $2}')"; if ((m<S)); then "${SUDO[@]}" fallocate -l "${S}M" /swapfile; "${SUDO[@]}" chmod 600 /swapfile; "${SUDO[@]}" mkswap /swapfile >/dev/null; "${SUDO[@]}" swapon /swapfile; grep -qF '/swapfile none swap sw 0 0' /etc/fstab || echo '/swapfile none swap sw 0 0' | "${SUDO[@]}" tee -a /etc/fstab >/dev/null; fi
