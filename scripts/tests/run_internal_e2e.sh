...
INTERNAL_WEB_PORT="${E2E_INTERNAL_WEB_PORT:-5005}"
CUSTOMER_WEB_PORT="${E2E_COMPLETION_WEB_PORT:-5006}"
...
case "${SITE,,}" in
  localhost|*.local) ;;
  *) fail "Phase 3 control actor is restricted to a local site; got $SITE." ;;
esac
...
control_env=(
  ...
  "E2E_INTERNAL_USERNAME=$E2E_INTERNAL_USERNAME"
)
...
echo "=== INTERNAL E2E PREFLIGHT ==="
preflight_output="$(run_control omc_app.e2e_internal_control.preflight)"
printf '%s\n' "$preflight_output"
request_id="$(extract_marker "$preflight_output" OMC_E2E_REQUEST_ID)"
task_id="$(extract_marker "$preflight_output" OMC_E2E_TASK_ID)"
[ -n "$request_id" ] || fail "Internal preflight did not return a request marker."
[ -n "$task_id" ] || fail "Internal preflight did not return a task marker."
...
if ! curl -fsS --max-time 2 "http://127.0.0.1:$DRIVER_PORT/status" >/dev/null 2>&1; then
  ...
  driver_log="$(mktemp "${TMPDIR:-/tmp}/omc-internal-e2e-chromedriver.XXXXXX.log")"
...
run_flutter() {
  ...
  (
    ...
    "$FLUTTER_BIN" drive \
      ...
      --dart-define="E2E_USERNAME=$username" \
...
echo "=== INTERNAL E2E LEG A: REAL TASK VISIBILITY + READ-ONLY AUTHORITY ==="
run_flutter \
...
echo "=== INTERNAL E2E ERP ACTOR: DOCUMENT REVIEW + TASK LIFECYCLE ==="
progress_output="$(run_control omc_app.e2e_internal_control.approve_documents_and_complete_work)"
printf '%s\n' "$progress_output"
progress_request="$(extract_marker "$progress_output" OMC_E2E_REQUEST_ID)"
progress_task="$(extract_marker "$progress_output" OMC_E2E_TASK_ID)"
[ "$progress_request" = "$request_id" ] || fail "ERP actor changed the request identity."
[ "$progress_task" = "$task_id" ] || fail "ERP actor changed the Task identity."

echo "=== INTERNAL E2E LEG B: CUSTOMER COMPLETION PROJECTION ==="
run_flutter \
