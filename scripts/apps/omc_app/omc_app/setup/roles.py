...
ACTIVE_STAFF_ROLES = {ADMIN_ROLE, MANAGER_ROLE, SUPPORT_AGENT_ROLE, DOCUMENT_REVIEWER_ROLE, FINANCE_REVIEWER_ROLE, CONSULTANT_ROLE, TAX_ASSOCIATE_ROLE, BUSINESS_PARTNER_ROLE}
ACTIVE_OMC_ROLES = ACTIVE_PORTAL_ROLES | ACTIVE_STAFF_ROLES
MANAGER_BLOCKED_DOCTYPES = {'OMC Branding Settings', 'OMC Mobile Settings', 'OMC Mobile Quick Action', 'OMC Service', 'OMC Service Category', 'OMC Service Form Field', 'OMC Service Required Document', 'OMC Service Stage Template', 'OMC App Banner', 'OMC Onboarding Slide', 'OMC FAQ', 'OMC Knowledge Article', 'OMC Announcement', 'OMC Expense Category', 'OMC Payment Account', 'OMC Tax Adjustment Rule', 'OMC Tax Calculator Settings', 'OMC Tax Input Field', 'OMC Tax Result Insight', 'OMC Tax Year'}
SPECIALIST_DOCTYPE_ACCESS = {SUPPORT_AGENT_ROLE: {'OMC Lead': {'read': 1, 'write': 1, 'create': 1}, 'OMC Support Ticket': {'read': 1, 'write': 1, 'create': 1}, 'OMC Support Ticket Message': {'read': 1, 'write': 1, 'create': 1}, 'OMC Customer Profile': {'read': 1}, 'OMC Referral': {'read': 1, 'create': 1}, 'OMC Manual Customer': {'read': 1, 'write': 1, 'create': 1}, 'OMC Service Request': {'read': 1, 'create': 1}, 'OMC Notification': {'read': 1, 'create': 1}}, DOCUMENT_REVIEWER_ROLE: {'OMC Service Document': {'read': 1, 'write': 1}, 'OMC Service Required Document': {'read': 1}, 'OMC Service Request': {'read': 1}, 'OMC Customer Profile': {'read': 1}, 'OMC Referral': {'read': 1}, 'OMC Manual Customer': {'read': 1, 'write': 1, 'create': 1}, 'OMC Service Timeline': {'read': 1, 'create': 1}}, FINANCE_REVIEWER_ROLE: {'OMC Service Payment': {'read': 1, 'write': 1}, 'OMC Payment Account': {'read': 1}, 'OMC Service Request': {'read': 1}, 'OMC Customer Profile': {'read': 1}, 'OMC Referral': {'read': 1}, 'OMC Manual Customer': {'read': 1, 'write': 1, 'create': 1}, 'OMC Service Timeline': {'read': 1, 'create': 1}}, CONSULTANT_ROLE: {'OMC Service Request': {'read': 1, 'write': 1}, 'OMC Service Document': {'read': 1}, 'OMC Customer Profile': {'read': 1}, 'OMC Referral': {'read': 1}, 'OMC Manual Customer': {'read': 1, 'write': 1, 'create': 1}, 'OMC Service Timeline': {'read': 1, 'create': 1}}, TAX_ASSOCIATE_ROLE: {'OMC Service Request': {'read': 1, 'write': 1}, 'OMC Service Document': {'read': 1}, 'OMC Customer Profile': {'read': 1}, 'OMC Referral': {'read': 1}, 'OMC Manual Customer': {'read': 1, 'write': 1, 'create': 1}, 'OMC Service Timeline': {'read': 1, 'create': 1}}, BUSINESS_PARTNER_ROLE: {'OMC Service Request': {'read': 1, 'write': 1}, 'OMC Service Document': {'read': 1}, 'OMC Customer Profile': {'read': 1}, 'OMC Referral': {'read': 1}, 'OMC Manual Customer': {'read': 1, 'write': 1, 'create': 1}, 'OMC Service Timeline': {'read': 1, 'create': 1}}}
...
PERMISSION_FIELDS = ('read', 'write', 'create', 'delete', 'submit', 'cancel', 'amend', 'report', 'export', 'import', 'print', 'email', 'share', 'if_owner', 'select')
...
def _available_permission_values(values):
    return {fieldname: int(value or 0) for fieldname, value in values.items() if fieldname in PERMISSION_FIELDS and _meta_has_field('DocPerm', fieldname)}

def _upsert_docperm(doctype, role, values):
    if not frappe.db.exists('DocType', doctype):
        return
    values = _available_permission_values(values)
    names = frappe.get_all('DocPerm', filters={'parent': doctype, 'role': role, 'permlevel': 0}, pluck='name', order_by='creation asc')
    if names:
...
def _base_permission():
    return {'read': 1, 'write': 0, 'create': 0, 'delete': 0, 'submit': 0, 'cancel': 0, 'amend': 0, 'report': 1, 'export': 1, 'import': 0, 'print': 1, 'email': 1, 'share': 0, 'if_owner': 0, 'select': 1}

def _admin_permission(is_submittable):
    values = _base_permission()
    values.update({'write': 1, 'create': 1, 'delete': 1, 'submit': 1 if is_submittable else 0, 'cancel': 1 if is_submittable else 0, 'amend': 1 if is_submittable else 0, 'import': 1, 'share': 1})
    return values
...
def _mobile_quick_action_manager_permission():
    return {'read': 0, 'write': 0, 'create': 0, 'delete': 0, 'submit': 0, 'cancel': 0, 'amend': 0, 'report': 0, 'export': 0, 'import': 0, 'print': 0, 'email': 0, 'share': 0, 'if_owner': 0, 'select': 0}

def _specialist_permission(values):
    permission = _base_permission()
    permission.update({'read': int(values.get('read', 0)), 'write': int(values.get('write', 0)), 'create': int(values.get('create', 0)), 'delete': 0, 'submit': 0, 'cancel': 0, 'amend': 0, 'report': int(values.get('read', 0)), 'export': 0, 'import': 0, 'print': int(values.get('read', 0)), 'email': 0, 'share': 0, 'if_owner': 0, 'select': int(values.get('read', 0))})
    return permission

def _omc_doctypes():
    rows = frappe.get_all('DocType', filters={'name': ['like', 'OMC %']}, fields=['name', 'istable', 'is_submittable'], order_by='name asc')
    return [row for row in rows if not int(row.istable or 0)]

def _remove_role_docperms(role_names):
    if not role_names:
        return
    for name in frappe.get_all('DocPerm', filters={'role': ['in', sorted(role_names)]}, pluck='name'):
...
def _migrate_legacy_user_roles():
    ...
    assignments = frappe.get_all('Has Role', filters={'role': ['in', sorted(LEGACY_ROLES)]}, fields=['parent', 'role'], order_by='parent asc')
    roles_by_user = {}
    ...
    for user_id, legacy_roles in roles_by_user.items():
        ...
        if legacy_roles.intersection(LEGACY_CLIENT_ROLES) and CUSTOMER_ROLE not in existing_roles:
            ...
            changed = True
        filtered_roles = [row for row in user_doc.roles or [] if row.role not in LEGACY_ROLES]
...
def _apply_permissions():
    ...
    for row in _omc_doctypes():
        ...
        admin_values = _mobile_quick_action_admin_permission() if doctype == 'OMC Mobile Quick Action' else _admin_permission(is_submittable)
