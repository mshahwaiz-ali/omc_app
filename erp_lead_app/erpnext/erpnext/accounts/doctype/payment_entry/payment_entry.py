...
class PaymentEntry(AccountsController):
	...
	def before_save(self):
		...
		if self.custom_structure_name and self.paid_amount:
			try:
				structure = frappe.get_doc("Sales Team Commission Structure", self.custom_structure_name)
				total_amount = self.paid_amount

				# Set Percentages
				self.custom_omc_percentage = structure.omc or 0
				self.custom_sales_person_percentage = structure.sales_person or 0
				self.custom_business_partner_consultant_percentage = structure.franchise or 0  # assuming franchise = BPC
				self.custom_reference_business_partner_percentage = structure.reference or 0

				# Set Calculated Amounts
				self.custom_omc_amount = total_amount * (self.custom_omc_percentage / 100)
				self.custom_sales_person_amount = total_amount * (self.custom_sales_person_percentage / 100)
				self.custom_business_partner_consultant_amount = total_amount * (self.custom_business_partner_consultant_percentage / 100)
				self.custom_reference_business_partner_amount = total_amount * (self.custom_reference_business_partner_percentage / 100)

			except Exception as e:
				frappe.log_error(frappe.get_traceback(), "Commission Amount Calculation Error")
	...
	def on_submit(self):
		...
		if not self.custom_structure_name or self.custom_omc_customer:
			return

		# Calculate distributable amount (excluding OMC)
		distributable_amount = flt(self.paid_amount) - flt(self.custom_omc_amount)
		...
		je.remark = f"Commission Distribution for Payment Entry {self.name}"
		je.user_remark = je.remark
		je.reference_number = self.name
		je.reference_date = self.posting_date

		# Credit total distributable amount from Commission Payable account
		je.append("accounts", {
			"account": "Commission on Sales - O",
			"debit_in_account_currency": distributable_amount,
			"reference_type": "Payment Entry",
			"reference_name": self.name
		})

		# Add Sales Person commission
		if self.custom_sales_person and flt(self.custom_sales_person_amount) > 0:
			je.append("accounts", {
				"account": "Commission Payable - O",
				"credit_in_account_currency": flt(self.custom_sales_person_amount),
				"party_type": self.custom_source,
				"party": self.custom_sales_person,
				"reference_type": "Payment Entry",
				"reference_name": self.name,
				"user_remark": "Sales Person Commission"
			})

		# Add Business Partner Consultant commission
		if self.custom_business_partner_consultant and flt(self.custom_business_partner_consultant_amount) > 0:
			je.append("accounts", {
				"account": "Commission Payable - O",
				"credit_in_account_currency": flt(self.custom_business_partner_consultant_amount),
				"party_type": "Consultant",
				"party": self.custom_business_partner_consultant,
				"reference_type": "Payment Entry",
				"reference_name": self.name,
				"user_remark": "Business Partner Consultant Commission"
			})

		# Add Reference Business Partner commission
		if self.custom_reference_business_partner and flt(self.custom_reference_business_partner_amount) > 0:
			je.append("accounts", {
				"account": "Commission Payable - O",
				"credit_in_account_currency": flt(self.custom_reference_business_partner_amount),
				"party_type": "Business Partner",
				"party": self.custom_reference_business_partner,
				"reference_type": "Payment Entry",
				"reference_name": self.name,
				"user_remark": "Reference Business Partner Commission"
			})
