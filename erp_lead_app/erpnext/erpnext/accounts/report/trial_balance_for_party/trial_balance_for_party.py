...
def get_data(filters, show_party_name):
	...
	# --- Logic for Category (Customer Group) Filter ---
	party_filters = {}
	
	# 1. Handle specific Party filter
	if filters.get("party"):
		party_filters["name"] = filters.get("party")
	
	# 2. Handle Customer Group filter (only if Party Type is Customer)
	if filters.get("party_type") == "Customer" and filters.get("customer_group"):
		party_filters["customer_group"] = filters.get("customer_group")
