...
class StockBalanceReport:
	...
	def prepare_item_warehouse_map(self, item_warehouse_map, entry, group_by_key):
		...
		# --- Custom logic for Stock Entry based on stock_entry_type ---
		if entry.voucher_type == "Stock Entry":
			stock_entry_type = frappe.db.get_value("Stock Entry", entry.voucher_no, "stock_entry_type")

			if stock_entry_type == "Manufacture":
				if qty_diff < 0:  # Outgoing
					qty_dict.custom_qty_out_manufacturing = qty_dict.get("custom_qty_out_manufacturing", 0) + abs(qty_diff)
					qty_dict.custom_value_out_manufacturing = qty_dict.get("custom_value_out_manufacturing", 0) + abs(value_diff)
				else:  # Incoming
					qty_dict.custom_qty_in_manufacturing = qty_dict.get("custom_qty_in_manufacturing", 0) + abs(qty_diff)
					qty_dict.custom_value_in_manufacturing = qty_dict.get("custom_value_in_manufacturing", 0) + abs(value_diff)

			elif stock_entry_type == "Repack":
				if qty_diff < 0:  # Outgoing
					qty_dict.custom_qty_out_repack__sub_assembly = qty_dict.get("custom_qty_out_repack__sub_assembly", 0) + abs(qty_diff)
					qty_dict.custom_value_out_repack__sub_assembly = qty_dict.get("custom_value_out_repack__sub_assembly", 0) + abs(value_diff)
				else:  # Incoming
					qty_dict.custom_qty_in_repack_sub_assembly = qty_dict.get("custom_qty_in_repack_sub_assembly", 0) + abs(qty_diff)
					qty_dict.custom_value_in_repack_sub_assembly = qty_dict.get("custom_value_in_repack_sub_assembly", 0) + abs(value_diff)

		# --- Existing opening / in / out calculations ---
	...
	def get_columns(self):
		...
		columns.extend(
			[
				...
				{
					"label": _("Qty out Manufacturing"),
					"fieldname": "custom_qty_out_manufacturing",
					"fieldtype": "Float",
					"width": 90,
				},
				{
					"label": _("Value out Manufacturing"),
					"fieldname": "custom_value_out_manufacturing",
					"fieldtype": "Currency",
					"width": 90,
				},
				{
					"label": _("Qty out (Repack + Sub Assembly)"),
					"fieldname": "custom_qty_out_repack__sub_assembly",
					"fieldtype": "Float",
					"width": 90,
				},
				{
					"label": _("Value out (Repack + Sub Assembly)"),
					"fieldname": "custom_value_out_repack__sub_assembly",
					"fieldtype": "Currency",
					"width": 90,
				},
				{
					"label": _("Qty In (Manufacturing)"),
					"fieldname": "custom_value_in_manufacturing",
					"fieldtype": "Float",
					"width": 90,
				},
				{
					"label": _("Value In (Manufacturing)"),
					"fieldname": "custom_qty_in_manufacturing",
					"fieldtype": "Currency",
					"width": 90,
				},
				{
