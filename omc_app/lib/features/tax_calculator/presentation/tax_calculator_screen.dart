...
class _TaxCalculatorScreenState extends ConsumerState<TaxCalculatorScreen> {
  ...
  Future<void> _loadConfig() async {
    ...
    try {
      ...
      setState(() {
        _loadingConfig = false;
        _error = 'Unable to load calculator configuration right now.';
      });
    }
  }

  void _syncDynamicFields(TaxCalculatorConfig config) {
    for (final field in [...config.simpleFields, ...config.advancedFields]) {
  ...
  Future<void> _calculate() async {
    ...
    final incomeAmount = double.tryParse(_incomeController.text.replaceAll(',', '').trim());
  ...
  Future<void> _downloadEstimatePdf() async {
    final log = _savedCalculationLog();
  ...
  Future<void> _shareEstimateWithConsultant() async {
    final log = _savedCalculationLog();
    if (log.isEmpty) return;
    setState(() => _sharingEstimate = true);
    try {
      ...
      if (mounted) setState(() => _sharingEstimate = false);
    }
  }
  ...
  Future<void> _startService() async {
    ...
    try {
      final response = await ref.read(taxCalculationRepositoryProvider).startServiceFromCalculation(
            calculationLog: log,
            service: service,
          );
      if (!mounted) return;
      ...
      child: ListView(
        ...
        children: [
          ...
          if (_loadingConfig)
            ...
            _ErrorCard(message: _error!, onRetry: _loadConfig)
            ...
            if (_result == null)
              ...
              _EstimateActionsCard(
                ...
                isSavingPdf: _savingPdf,
                isSharing: _sharingEstimate,
              ...
              if (_result!.recommendedNextSteps.isNotEmpty || config.recommendedNextSteps.isNotEmpty) ...[
                const SizedBox(height: 14),
                _NextStepsCard(
                  steps: _result!.recommendedNextSteps.isNotEmpty ? _result!.recommendedNextSteps : config.recommendedNextSteps,
                ),
              ],
              ...
              const SizedBox(height: 14),
              _CtaCard(cta: _result!.cta, isLoading: _startingService, onPressed: _startService),
              const SizedBox(height: 14),
        ...
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              labelText: incomeMode == TaxIncomeMode.monthly ? 'Monthly income' : 'Annual income',
              hintText: incomeMode == TaxIncomeMode.monthly ? 'Example: 250000' : 'Example: 3000000',
  ...
  Widget build(BuildContext context) {
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        prefixIcon: field.inputType == 'number' ? const Icon(Icons.add_card_outlined) : const Icon(Icons.text_fields_rounded),
      ),
    );
  }
}
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            style: const TextStyle(color: AppTheme.textPrimary, fontSize: 30, fontWeight: FontWeight.w900),
          ),
          const SizedBox(height: 16),
          Wrap(
            spacing: 10,
...
class _EstimateActionsCard extends StatelessWidget {
  const _EstimateActionsCard({
    required this.canUseSavedActions,
      ...
      child: Column(
        ...
        children: [
          const _SectionHeader(
            ...
            subtitle: 'Save, share, or review previous tax estimates.',
          ...
          Wrap(
            ...
            children: [
              OutlinedButton.icon(
                ...
                icon: Icon(isSavingPdf ? Icons.hourglass_empty_rounded : Icons.picture_as_pdf_outlined),
                label: Text(isSavingPdf ? 'Saving PDF...' : 'Save PDF'),
              ),
              OutlinedButton.icon(
                onPressed: isSharing ? null : onShare,
                icon: Icon(isSharing ? Icons.hourglass_empty_rounded : Icons.support_agent_rounded),
                label: Text(isSharing ? 'Sharing...' : 'Share with Consultant'),
              ),
          ...
          if (!canUseSavedActions) ...[
            ...
            const Text(
              'Login estimates are saved to backend. Guest estimates can be viewed but not downloaded or shared.',
              style: TextStyle(color: AppTheme.textSecondary, fontSize: 12, height: 1.35, fontWeight: FontWeight.w600),
            ),
          ],
        ],
      ),
            ...
            child: Column(
              ...
              children: [
                Text(
                  ...
                  style: const TextStyle(color: AppTheme.textPrimary, fontSize: 17, fontWeight: FontWeight.w900),
                ),
                const SizedBox(height: 6),
                Text(
                  health.reason,
                  style: const TextStyle(color: AppTheme.textSecondary, height: 1.35, fontWeight: FontWeight.w600),
                ),
              ],
            ),
          ),
...
class _InsightsCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _ResultSection(
      ...
      children: [for (final insight in insights) _InsightTile(insight: insight)],
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(insight.title, style: const TextStyle(color: AppTheme.textPrimary, fontWeight: FontWeight.w900)),
          const SizedBox(height: 4),
          Text(insight.message, style: const TextStyle(color: AppTheme.textSecondary, height: 1.35, fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
...
class _NextStepsCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _ResultSection(
      ...
      children: [for (var i = 0; i < steps.length; i++) _ResultRow(label: '${i + 1}', value: steps[i])],
    );
  }
}
...
class _RequiredDocumentsCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _ResultSection(
      ...
      children: [for (final document in documents) _ResultRow(label: 'Required', value: document)],
    );
  }
}
      ...
      children: [
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(title, style: const TextStyle(color: AppTheme.textPrimary, fontSize: 17, fontWeight: FontWeight.w900)),
              if (subtitle.isNotEmpty) ...[
                const SizedBox(height: 3),
                Text(subtitle, style: const TextStyle(color: AppTheme.textSecondary, fontSize: 12, height: 1.3, fontWeight: FontWeight.w600)),
              ],
            ],
          ),
        ),
    ...
    return SizedBox(
      ...
      child: Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: AppTheme.textPrimary, fontSize: 15, fontWeight: FontWeight.w900)),
            const SizedBox(height: 4),
            Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: AppTheme.textSecondary, fontSize: 11, fontWeight: FontWeight.w700)),
          ],
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          SizedBox(width: 118, child: Text(label, style: const TextStyle(color: AppTheme.textSecondary, fontWeight: FontWeight.w700))),
          const SizedBox(width: 10),
          Expanded(child: Text(value, textAlign: TextAlign.right, style: const TextStyle(color: AppTheme.textPrimary, fontWeight: FontWeight.w900))),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(label, style: const TextStyle(color: AppTheme.primaryRed, fontSize: 11, fontWeight: FontWeight.w900)),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          const SizedBox(width: 10),
          Expanded(child: Text(text, style: const TextStyle(color: AppTheme.textPrimary, fontSize: 12, height: 1.35, fontWeight: FontWeight.w700))),
        ],
      ),
    );
  }
      ...
      child: Column(
        children: [
          Container(
            ...
            child: const Icon(Icons.receipt_long_outlined, size: 30, color: AppTheme.primaryRed),
          ),
          const SizedBox(height: 14),
          ...
          const Text(
            ...
            style: TextStyle(color: AppTheme.textSecondary, height: 1.4, fontWeight: FontWeight.w600),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    return PremiumCard(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          Text(message, style: const TextStyle(color: AppTheme.textPrimary, fontWeight: FontWeight.w800)),
          const SizedBox(height: 14),
