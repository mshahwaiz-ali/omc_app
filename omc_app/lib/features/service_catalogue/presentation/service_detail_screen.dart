                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      FilledButton.icon(
                        ...
                        label: Text(_startRequestLabel(service)),
...
class _OverviewCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final overview = (service.description ?? service.shortDescription ?? 'No overview available.').trim();
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              _MiniNote(label: 'Support-assisted review', color: _primary),
              _MiniNote(label: 'Progress tracked live', color: _review),
            ],
          ),
        ],
      ),
...
_Tone _serviceTone(ServiceItem service) {
  final source = '${service.category} ${service.title} ${service.wizardType ?? ''}'.toLowerCase();

  if (source.contains('visa')) {
    return const _Tone(icon: Icons.flight_takeoff_rounded, color: _success);
  }
  if (source.contains('tax') || source.contains('ntn') || source.contains('gst')) {
    return const _Tone(icon: Icons.receipt_long_outlined, color: Color(0xFF4F46E5));
  }
  if (source.contains('business') || source.contains('setup')) {
    return const _Tone(icon: Icons.apartment_outlined, color: Color(0xFF7C3AED));
  }
  if (source.contains('document')) {
    return const _Tone(icon: Icons.description_outlined, color: Color(0xFF6366F1));
  }
  if (source.contains('payment') || source.contains('receipt') || source.contains('invoice')) {
    return const _Tone(icon: Icons.payments_outlined, color: Color(0xFF059669));
  }
  ...
  if (source.contains('lead')) {
    return const _Tone(icon: Icons.record_voice_over_rounded, color: Color(0xFF8B5CF6));
  }
  if (source.contains('task') || source.contains('todo')) {
    return const _Tone(icon: Icons.task_alt_rounded, color: Color(0xFFF97316));
  }
  if (source.contains('support') || source.contains('case') || source.contains('request')) {
    return const _Tone(icon: Icons.support_agent_rounded, color: _primary);
  }
  return const _Tone(icon: Icons.workspace_premium_outlined, color: _primary);
}
...
String _startRequestLabel(ServiceItem service) {
  return _wizardBadgeLabel(service) == null ? 'Request service' : 'Start wizard';
}
