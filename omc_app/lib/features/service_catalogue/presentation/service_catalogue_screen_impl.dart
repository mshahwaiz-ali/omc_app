            ...
            child: ListView(
              ...
              children: [
                _Header(
                  ...
                  unreadCount: _canOpenNotifications(capabilities) ? services.length : 0,
                  authState: authState,
                  onNotificationsTap: () {
  ...
  List<ServiceItem> _filterServices(List<ServiceItem> services) {
    ...
    for (var index = 0; index < services.length; index++) {
      ...
      final matchesStatus = _selectedStatus == _allStatus ||
          serviceCatalogueStatusLabelFor(status) == _selectedStatus;
...
class _SummaryGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Wrap(
      ...
      runSpacing: 10,
      children: [
        _SummaryTile(icon: Icons.folder_outlined, label: 'Services', value: totalServices.toString(), subtitle: 'Total', tint: _primary),
        _SummaryTile(icon: Icons.visibility_outlined, label: 'Open', value: openServices.toString(), subtitle: 'Active cases', tint: _primary),
...
class _StatusChipsRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final items = <({String label, Color color, int count})>[
      (label: 'All Services', color: _primary, count: counts.values.fold<int>(0, (a, b) => a + b)),
            ...
            children: [
              ...
              if (count != null) ...[
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: selected ? selectedColor.withValues(alpha: 0.14) : _surfaceSoft,
                    borderRadius: BorderRadius.circular(999),
                  ),
                  child: Text(
                    count.toString(),
...
class _ServiceDashboardCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final visibleRequirements = service.requirements.take(3).toList(growable: false);
    ...
    final statusColor = serviceCatalogueStatusColor(status);
    final progress = serviceCatalogueStatusProgress(status, service.id.hashCode.abs());
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  const Icon(Icons.chevron_right_rounded, color: _slate, size: 28),
                ],
              ),
            ],
          ),
...
_BannerData bannerFor(AuthState authState) {
  ...
  return const _BannerData(
    ...
    message: 'Track requests, upload documents and review progress without leaving this page.',
...
String lockedAccessMessage(AuthCapabilities capabilities) {
  if (capabilities.isGuest) return 'Please sign in or create an account to continue.';
...
bool canTrackServices(AuthCapabilities capabilities) {
  return capabilities.canTrackRequests ||
      capabilities.canViewCustomerDashboard ||
      capabilities.canAccessCustomerDashboard ||
      capabilities.canAccessInternalWorkspace;
...
bool canOpenNotifications(AuthCapabilities capabilities) {
  return capabilities.canViewCustomerNotifications ||
      capabilities.isApproved ||
      capabilities.isInternal ||
      capabilities.canAccessInternalWorkspace;
