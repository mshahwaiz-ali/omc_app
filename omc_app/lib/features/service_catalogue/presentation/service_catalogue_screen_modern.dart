...
const Color _mutedSurface = Color(0xFFF8FAFC);
const Color _softBorder = Color(0xFFE5E7EB);
const Color _open = Color(0xFF334155);
const Color _review = Color(0xFF0F766E);
...
_StatusBannerData _bannerFor(AuthState authState) {
  ...
  if (authState.capabilities.isRejected) {
    return const _StatusBannerData(
      ...
      border: _roseBorder,
      iconBackground: Color(0xFFFFFBFC),
...
class _StatusChipsRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final items = <({String label, Color color, int count})>[
      ...
      (label: 'Open', color: _open, count: counts[_ServiceStatus.open] ?? 0),
      ...
      child: InkWell(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(16),
            border: Border.all(
              color: selected ? selectedColor.withValues(alpha: 0.16) : Colors.black.withValues(alpha: 0.08),
            ),
            boxShadow: [
              BoxShadow(
            ...
            children: [
              ...
              if (count != null) ...[
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: selected ? selectedColor.withValues(alpha: 0.12) : _mutedSurface,
                    borderRadius: BorderRadius.circular(999),
                  ),
                  child: Text(
                    count.toString(),
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 10),
              _SupportIconButton(onPressed: onWhatsApp),
            ],
          ),
        ],
      ),
...
class _SupportIconButton extends StatelessWidget {
  const _SupportIconButton({required this.onPressed});
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: OutlinedButton.icon(
        onPressed: onPressed,
        icon: const Icon(Icons.tune_rounded, size: 18, color: _ink),
        label: Text(label),
        style: OutlinedButton.styleFrom(
          padding: const EdgeInsets.symmetric(horizontal: 16),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18)),
          side: BorderSide(color: _ink.withValues(alpha: 0.14)),
          foregroundColor: _ink,
