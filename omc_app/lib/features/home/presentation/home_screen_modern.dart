...
class HomeScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final quickActions = _homeActions(
    ...
    final isLockedProfile = capabilities.isGuest || capabilities.isPending || capabilities.isRejected;
              ...
              child: CustomScrollView(
                ...
                slivers: [
                  ...
                  SliverPadding(
                    ...
                    sliver: SliverToBoxAdapter(
                      child: _HeroPanel(
                        ...
                        summary: summary,
                        capabilities: capabilities,
                        onPrimary: () => _openHeroPrimary(context, summary, capabilities),
                        onSecondary: () => _openHeroSecondary(
                          context,
                          onOpenCalculator,
                        ),
                        avatarUrl: avatarUrl,
                  ...
                  SliverPadding(
                    ...
                    sliver: SliverToBoxAdapter(
                      child: _SectionHeader(
                        title: summary.nextAction == null ? 'What is next' : 'Your next step',
                        actionLabel: summary.nextAction == null ? null : summary.nextAction!.buttonLabel,
                        onAction: summary.nextAction == null
                            ? null
                            : () => _goAllowed(
                                context: context,
                                route: summary.nextAction!.route,
                                capabilities: capabilities,
                                requiredCapability: _routeCapability(summary.nextAction!.route),
                              ),
                      ),
                    ),
                  ),
                  SliverPadding(
                    padding: const EdgeInsets.symmetric(horizontal: 16),
                    sliver: SliverToBoxAdapter(
                      child: _NextActionPanel(
                        summary: summary,
                        capabilities: capabilities,
                        onOpen: () => _openNextAction(context, summary, capabilities, onOpenServices),
                      ),
                    ),
                  ),
                  SliverPadding(
                  ...
                  SliverPadding(
                    ...
                    sliver: SliverToBoxAdapter(
                      child: _SectionHeader(
                        title: summary.serviceSnapshots.isEmpty ? 'Start something new' : 'Your active services',
  ...
  List<_MetricItem> _metrics(HomeDashboardSummary summary) {
    return [
      _MetricItem(
  ...
  String _accessMessage(AuthCapabilities capabilities) {
    if (capabilities.isGuest) {
      return 'Please sign in or create an account to unlock this area.';
    }
    if (capabilities.isPending) {
      return 'Your account is under review. OMC will enable access after approval.';
    }
    if (capabilities.isRejected) {
      return 'This account is not approved for that action.';
    }
        ...
        child: Stack(
          children: const [
            Positioned(
              top: -120,
              left: -90,
              child: _GlowBlob(size: 240, colors: [Color(0x22C81D32), Color(0x06C81D32)]),
            ),
            Positioned(
              top: 140,
              right: -110,
              child: _GlowBlob(size: 220, colors: [Color(0x1A4F8DFD), Color(0x044F8DFD)]),
            ),
            Positioned(
              bottom: -120,
              left: -80,
              child: _GlowBlob(size: 220, colors: [Color(0x1A17B890), Color(0x0417B890)]),
            ),
          ],
        ),
      ),
          ...
          child: Column(
            ...
            children: [
              ...
              Wrap(
                ...
                runSpacing: 8,
                children: const [
                  _MiniChip(label: 'Workspace live', icon: Icons.bolt_rounded),
                  _MiniChip(label: 'Fast access', icon: Icons.flash_on_rounded),
                ],
              ),
            ],
          ),
...
class _AvatarBadge extends StatelessWidget {
  ...
  String _initials(String name) {
    final parts = name.trim().split(' ').where((item) => item.isNotEmpty).toList(growable: false);
...
class _AccessNotice extends StatelessWidget {
  const _AccessNotice({required this.capabilities, required this.onPrimary});
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(
              capabilities.isRejected ? Icons.block_rounded : Icons.verified_outlined,
              color: tone,
            ),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  capabilities.isRejected ? 'Profile blocked' : 'Profile under review',
...
class _HeroPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final heroSubtitle = nextAction?.subtitle ??
        summary.fallbackMessage ??
        'Explore services, track progress, and keep the workspace moving.';
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: const [
          BoxShadow(color: Color(0x220F172A), blurRadius: 28, offset: Offset(0, 16)),
        ],
      ),
      child: Stack(
        children: [
                  ...
                  children: [
                    Expanded(
                      child: ElevatedButton(
                        ...
                        onPressed: onPrimary,
                        child: Text(nextAction?.buttonLabel.isNotEmpty == true ? nextAction!.buttonLabel : 'Continue'),
                      ),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
...
class _QuickActionCard extends StatelessWidget {
  const _QuickActionCard({required this.action, required this.allowed, required this.onTap});
...
class _NextActionPanel extends StatelessWidget {
  const _NextActionPanel({required this.summary, required this.capabilities, required this.onOpen});
  ...
  Widget build(BuildContext context) {
    final nextAction = summary.nextAction;
    if (nextAction == null) {
      return _EmptyNextActionCard(onOpen: onOpen);
    }
          ...
          child: Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.play_circle_outline_rounded, color: AppTheme.primaryRed, size: 28),
              ),
              const SizedBox(width: 14),
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      nextAction.type.isNotEmpty ? nextAction.type.toUpperCase() : 'NEXT',
                      style: const TextStyle(
                        fontSize: 11,
                        fontWeight: FontWeight.w900,
              ...
              Column(
                ...
                children: [
                  Text(
                    nextAction.buttonLabel.isEmpty ? 'Open' : nextAction.buttonLabel,
                    style: const TextStyle(fontWeight: FontWeight.w800, color: AppTheme.primaryRed),
                  ),
                  const SizedBox(height: 2),
                  const Icon(Icons.chevron_right_rounded, color: AppTheme.primaryRed),
                ],
              ),
            ],
          ),
...
class _EmptyNextActionCard extends StatelessWidget {
  const _EmptyNextActionCard({required this.onOpen});
          ...
          child: Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.space_dashboard_outlined, color: AppTheme.textSecondary),
              ),
              const SizedBox(width: 14),
              const Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 4),
                    Text(
                      'Open the catalogue, check progress, or start a new request from one place.',
...
class _ServicePanel extends StatelessWidget {
  const _ServicePanel({required this.services, required this.onOpen, required this.onOpenServices});
