...
class HomeScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final actions = _actions(actionsAsync.maybeWhen(data: (value) => value, orElse: () => fallbackMobileQuickActions), capabilities.canAccessInternalWorkspace);
                ...
                slivers: [
                  ...
                  SliverPadding(
                    ...
                    sliver: SliverToBoxAdapter(
                      child: _SectionTitle(
                        title: 'At a glance',
                        action: summary.serviceSnapshots.isEmpty ? null : 'Track',
                  ...
                  SliverPadding(
                    padding: const EdgeInsets.symmetric(horizontal: 16),
                    sliver: SliverGrid(
                      delegate: SliverChildListDelegate.fixed(_metricCards(summary)),
                  ...
                  SliverPadding(
                    ...
                    sliver: SliverToBoxAdapter(
                      child: _SectionTitle(
                        ...
                        action: summary.serviceSnapshots.isEmpty ? null : 'View all',
                        onTap: summary.serviceSnapshots.isEmpty
                            ? null
                            : () => _goAllowed(
                                context: context,
  ...
  List<MobileQuickAction> _actions(List<MobileQuickAction> source, bool allowInternal) {
  ...
  List<_MetricCardModel> _metricCards(HomeDashboardSummary summary) => [
        _MetricCardModel('Active services', summary.activeCases, Icons.work_outline_rounded, const Color(0xFF4F8DFD), 'Live cases'),
        _MetricCardModel('Documents', summary.pendingDocuments, Icons.description_outlined, const Color(0xFF17B890), 'Pending review'),
        _MetricCardModel('Payments due', summary.paymentsDue, Icons.payments_outlined, const Color(0xFFF59E0B), 'Action needed'),
        _MetricCardModel('Notifications', summary.unreadNotifications, Icons.notifications_none_rounded, const Color(0xFF8B5CF6), 'Unread'),
      ];
  ...
  void _openNext(
    ...
    if (next != null && next.route.trim().isNotEmpty) {
      _goAllowed(
        context: context,
        route: next.route,
        capabilities: capabilities,
        requiredCapability: _routeCapability(next.route),
      );
      return;
    }
    final callback = onOpenServices;
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(displayName, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 27, height: 1.05, fontWeight: FontWeight.w900, color: AppTheme.textPrimary, letterSpacing: -0.4)),
        ...
        _BellButton(unreadNotifications: unreadNotifications, onTap: onNotifications),
...
class _BellButton extends StatelessWidget {
  const _BellButton({required this.unreadNotifications, required this.onTap});
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: Stack(
          ...
          children: [
            const SizedBox(width: 48, height: 48, child: Icon(Icons.notifications_none_rounded, size: 22, color: AppTheme.textPrimary)),
            if (unreadNotifications > 0)
              Positioned(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(color: const Color(0xFFE11D48), borderRadius: BorderRadius.circular(99)),
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 12),
              const Expanded(
                child: Text('Search services, documents, invoices...', style: TextStyle(color: AppTheme.textMuted, fontSize: 14, fontWeight: FontWeight.w500)),
              ),
              Container(
                width: 34,
                height: 34,
...
class _AccessCard extends StatelessWidget {
  const _AccessCard({required this.capabilities, required this.onPrimary});
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(capabilities.isRejected ? 'Profile blocked' : 'Profile under review', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: AppTheme.textPrimary)),
                const SizedBox(height: 4),
                Text(
                  capabilities.isRejected ? 'Please contact OMC support for manual review.' : 'You can explore public services. Full access unlocks after approval.',
...
class _HeroCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final next = summary.nextAction;
    final title = next?.title ?? 'Start with OMC';
    final subtitle = next?.subtitle ?? summary.fallbackMessage ?? 'Explore services, track progress, and keep the workspace moving.';

    return Container(
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(28),
        gradient: const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Color(0xFF0F172A), Color(0xFF1E293B), Color(0xFFC81D32)]),
        boxShadow: const [BoxShadow(color: Color(0x220F172A), blurRadius: 28, offset: Offset(0, 16))],
      ),
      child: Stack(
        children: [
          Positioned(top: -48, right: -10, child: _Glow(size: 140, color: Colors.white.withOpacity(0.08))),
          Positioned(bottom: -56, left: -26, child: _Glow(size: 132, color: Colors.white.withOpacity(0.07))),
          Padding(
              ...
              children: [
                Row(
                  children: [
                    Container(
                      ...
                      child: Text(
                        capabilities.isGuest ? 'Guest workspace' : capabilities.isPending ? 'Under review' : capabilities.isRejected ? 'Action restricted' : 'Priority workspace',
                        style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w700),
                      ),
                    ),
                    const Spacer(),
                    if (avatarUrl != null)
                      _AvatarMini(url: avatarUrl!)
                    else
                      const _AvatarMiniFallback(),
                  ],
                ),
                const SizedBox(height: 18),
                Text('Good to see you,', style: TextStyle(color: Colors.white.withOpacity(0.78), fontSize: 14, fontWeight: FontWeight.w600)),
                const SizedBox(height: 4),
                Text(name, style: const TextStyle(color: Colors.white, fontSize: 26, height: 1.05, fontWeight: FontWeight.w900, letterSpacing: -0.5)),
                const SizedBox(height: 12),
                Text(subtitle, style: TextStyle(color: Colors.white.withOpacity(0.86), fontSize: 13.5, height: 1.45, fontWeight: FontWeight.w500)),
                ...
                if ((summary.fallbackMessage ?? '').trim().isNotEmpty) ...[
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(color: Colors.white.withOpacity(0.10), borderRadius: BorderRadius.circular(18), border: Border.all(color: Colors.white.withOpacity(0.14))),
                    child: Text(summary.fallbackMessage!, style: TextStyle(color: Colors.white.withOpacity(0.84), fontSize: 12.5, height: 1.35)),
                  ),
                ],
                  ...
                  children: [
                    Expanded(
                      child: ElevatedButton(
                        style: ElevatedButton.styleFrom(backgroundColor: Colors.white, foregroundColor: AppTheme.primaryRed, elevation: 0, padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18)), textStyle: const TextStyle(fontWeight: FontWeight.w800, fontSize: 14)),
                        onPressed: onPrimary,
                        child: Text(next?.buttonLabel.trim().isNotEmpty == true ? next!.buttonLabel : 'Continue'),
                      ),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: OutlinedButton(
                        style: OutlinedButton.styleFrom(foregroundColor: Colors.white, side: BorderSide(color: Colors.white.withOpacity(0.32)), padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18)), textStyle: const TextStyle(fontWeight: FontWeight.w800, fontSize: 14)),
...
class _AvatarMini extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      width: 38,
      height: 38,
      decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Colors.white.withOpacity(0.28), width: 1.2)),
...
class _AvatarMiniFallback extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      width: 38,
      height: 38,
      decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.16), border: Border.all(color: Colors.white.withOpacity(0.20), width: 1.2)),
      alignment: Alignment.center,
...
class _HeroStat extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white.withOpacity(0.10), borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.white.withOpacity(0.14))),
      child: Column(
        mainAxisSize: MainAxisSize.min,
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          Text(label, style: TextStyle(color: Colors.white.withOpacity(0.72), fontSize: 11, fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
...
class _QuickActionsRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (actions.isEmpty) {
      return const SizedBox(height: 124, child: Center(child: Text('No quick actions available yet.', style: TextStyle(color: AppTheme.textMuted, fontWeight: FontWeight.w600))));
    }
            ...
            child: Column(
              ...
              children: [
                Container(
                  width: 40,
                  height: 40,
                  decoration: BoxDecoration(color: palette.soft, borderRadius: BorderRadius.circular(14)),
                  padding: const EdgeInsets.all(8.5),
                  child: SvgPicture.asset(action.iconAsset, fit: BoxFit.contain),
                ),
                const Spacer(),
                Text(action.title, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w900, color: AppTheme.textPrimary, height: 1.1)),
                const SizedBox(height: 4),
                Text(action.subtitle, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 11.5, fontWeight: FontWeight.w600, color: AppTheme.textSecondary)),
                const SizedBox(height: 8),
                _Pill(label: allowed ? 'Open' : 'Locked', color: allowed ? palette.accent : AppTheme.textMuted),
              ],
            ),
          ),
        ),
...
class _MetricCardModel {
  const _MetricCardModel(this.label, this.value, this.icon, this.color, this.hint);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(width: 36, height: 36, decoration: BoxDecoration(color: item.color.withOpacity(0.12), borderRadius: BorderRadius.circular(14)), child: Icon(item.icon, color: item.color, size: 20)),
              const Spacer(),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                decoration: BoxDecoration(color: item.color.withOpacity(0.10), borderRadius: BorderRadius.circular(999)),
                child: Text(item.hint, style: TextStyle(color: item.color, fontSize: 10, fontWeight: FontWeight.w800)),
              ),
            ],
          ),
          const Spacer(),
          ...
          const SizedBox(height: 2),
          Text(item.label, style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: AppTheme.textSecondary)),
        ],
      ),
    );
  }
        ...
        child: Container(
          ...
          child: Column(
            children: [
              Row(
                children: [
                  Container(width: 40, height: 40, decoration: BoxDecoration(color: color.withOpacity(0.12), borderRadius: BorderRadius.circular(14)), child: Icon(_statusIcon(service.status), color: color, size: 20)),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                  ...
                  _Pill(label: _statusLabel(service.status), color: color),
                ],
              ),
              const SizedBox(height: 14),
              ClipRRect(
                borderRadius: BorderRadius.circular(999),
                child: LinearProgressIndicator(minHeight: 8, value: service.progress.clamp(0, 1), backgroundColor: AppTheme.cardSoft, valueColor: AlwaysStoppedAnimation<Color>(color)),
              ),
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Container(width: 38, height: 38, decoration: BoxDecoration(color: color.withOpacity(0.12), borderRadius: BorderRadius.circular(14)), child: Icon(icon, color: color, size: 18)),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
