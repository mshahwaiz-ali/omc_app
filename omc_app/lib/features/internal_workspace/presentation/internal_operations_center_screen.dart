...
class _InternalOperationsCenterScreenState
  ...
  Widget build(BuildContext context) {
    if (widget.area == InternalOperationArea.payments) {
      return _buildPaymentReview(context);
    }
                    ...
                    children: [
                      ...
                      if (filtered.isEmpty)
                        const _OperationsEmpty(
                          ...
                          message:
                              'Adjust the filter or refresh the scoped payment queue.',
                        )
                      else
  ...
  List<PaymentItem> _filteredPayments(List<PaymentItem> payments) {
    ...
    Iterable<PaymentItem> result = payments;
    ...
    result = result.where((payment) {
      switch (_selectedFilter) {
        ...
        case 'Own':
          return payment.isOwnPayment;
        case 'Referrals':
          return payment.isReferralPayment;
        case 'All':
        default:
          return true;
      }
    ...
    sorted.sort((a, b) {
      final reviewA =
          a.status == PaymentStatus.receiptSubmitted ||
          a.status == PaymentStatus.underReview;
      final reviewB =
          b.status == PaymentStatus.receiptSubmitted ||
          b.status == PaymentStatus.underReview;
      final reviewCompare = (reviewB ? 1 : 0).compareTo(reviewA ? 1 : 0);
      ...
      return a.customerLabel.compareTo(b.customerLabel);
    });
    return sorted;
  }

  Map<String, int> _internalPaymentCounts(List<PaymentItem> payments) {
    int count(bool Function(PaymentItem payment) predicate) =>
        payments.where(predicate).length;
    ...
    return {
      ...
      'Action Required': count((payment) => payment.requiresAction),
      'Paid': count((payment) => payment.status == PaymentStatus.paid),
      'Rejected': count((payment) => payment.status == PaymentStatus.rejected),
      'Own': count((payment) => payment.isOwnPayment),
      'Referrals': count((payment) => payment.isReferralPayment),
      'All': payments.length,
    };
  }
      ...
      children: [
        TextField(
          ...
          decoration: InputDecoration(
            hintText: 'Search customer, payment or service reference',
...
class _InternalPaymentMetric extends StatelessWidget {
  const _InternalPaymentMetric({
    required this.label,
    required this.value,
    required this.icon,
  });
              ...
              Row(
                children: [
                  ...
                  Container(
                    ...
                    child: Text(
                      payment.isReferralPayment ? 'Referral' : 'Own',
                      style: const TextStyle(
                        color: AppTheme.textSecondary,
                        fontSize: 10,
                        fontWeight: FontWeight.w800,
                  ...
                  TextButton.icon(
                    ...
                    label: Text(isReview ? 'Review' : 'Open'),
                    style: TextButton.styleFrom(
                      foregroundColor: isReview
                          ? AppTheme.primary
                          : statusColor,
                      padding: const EdgeInsets.symmetric(
                        horizontal: 9,
