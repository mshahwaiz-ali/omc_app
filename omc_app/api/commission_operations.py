...
def _finance_capabilities() -> dict[str, Any]:
    ...
    if not (
        values.get("can_approve_commissions")
        or values.get("can_mark_commissions_paid")
    ):
        frappe.throw(
...
def _validated_evidence(value: Any) -> str:
    ...
    if not evidence:
        return ""
    normalized = next(
        (item for item in EVIDENCE_STATES if item.lower() == evidence.lower()),
        "",
    )
    if not normalized:
        frappe.throw(
            "Unsupported accounting evidence status.",
...
def _payload(
    ...
    *,
    ...
    evidence = _text(
        ...
        else getattr(row, "accounting_evidence_status", None)
    ) or "Unverified"
    ...
    return {
        ...
        "customer_name": (
            _text(request.get("customer_name"))
            or _text(getattr(row, "erp_customer", None))
        ),
        "service": _text(request.get("service")),
        "service_title": (
            _text(request.get("service_title"))
            or _text(request.get("service"))
        ),
        "currency": _text(getattr(row, "currency", None)) or "PKR",
        "basis_amount": getattr(row, "basis_amount", 0) or 0,
        "commission_percent": (
            getattr(row, "commission_percent_snapshot", 0) or 0
        ),
        ...
        "payable_marked_at": str(
            getattr(row, "payable_marked_at", None) or ""
        ),
        ...
        "settlement_reference": _text(
            getattr(row, "settlement_reference", None)
        ),
...
def _or_filters(search: str) -> dict[str, Any] | None:
    ...
    return {
        "name": ["like", pattern],
        "beneficiary": ["like", pattern],
        "beneficiary_user": ["like", pattern],
        "service_request": ["like", pattern],
        "erp_customer": ["like", pattern],
        "component": ["like", pattern],
    }


@frappe.whitelist()
def get_commission_allocations(
    status=None,
    evidence_status=None,
    search=None,
    limit_start=0,
    limit_page_length=DEFAULT_LIMIT,
):
    ...
    requests = _request_map(
        {
            _text(row.service_request)
            for row in page_rows
            if _text(row.service_request)
        }
    )
    ...
    return {
        "items": [
            _payload(
                row,
                request=requests.get(_text(row.service_request)),
                values=values,
            )
            for row in page_rows
        ],
...
def get_commission_allocation(allocation=None, name=None):
    ...
    if not allocation_name or not frappe.db.exists(DOCTYPE, allocation_name):
        frappe.throw(
            "Commission allocation was not found.",
