...
def _get_available_tax_years():
    rows = frappe.get_all(
        ...
        order_by="effective_from desc, tax_year desc",
    )
    return [
        {
            "name": row.name,
            "tax_year": row.tax_year or row.name,
            "title": row.title or row.tax_year or row.name,
            "currency": row.currency or "PKR",
            "verified": bool(row.last_verified_on),
            "last_verified_on": str(row.last_verified_on or ""),
            "public_note": row.public_note or "",
        }
        for row in rows
    ]
