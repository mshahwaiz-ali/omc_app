...
void main() {
  group('P3 closeout contracts', () {
    ...
    test(
      'splash has no artificial startup delay and respects reduced motion',
      () {
        final source = File(
    ...
    test('tax calculator uses canonical access and real config refresh', () {
      ...
      expect(
        source,
        contains("LoadingView(message: 'Loading tax calculator')"),
      );
    ...
    test(
      'tax calculator customer copy avoids backend implementation language',
      () {
        final source = File(
          'lib/features/tax_calculator/presentation/tax_calculator_screen.dart',
        ).readAsStringSync();
